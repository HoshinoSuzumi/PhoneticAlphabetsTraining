export const useRandomCallsign = (callSignLength: number = 3) => {
    const callsignTemplates: {
        [key: string]: string
    } = {
    ...
    const generateCallSign = (template: string) => {
        ...
        // const regex = /\$([c|n]\d?)\{([^}]+)\}/g
        // return template.replace(regex, (match, type, parma) => {
        //     const optionArray = parma.split('-');
        //     let generatedPart = '';
        //
        //     if (type.startsWith('c')) {
        //         // 字符
        //         const charCount = type === 'c' ? 1 : parseInt(type.slice(1), 10);
        //
        //         for (let i = 0; i < charCount; i++) {
        //             const charRanges = parma.split('|');
        //             const selectedRange = charRanges[Math.floor(Math.random() * charRanges.length)];
        //             const [start, end] = selectedRange.split('-');
        //             const randomCharCode = start.charCodeAt(0) + Math.floor(Math.random() * (end.charCodeAt(0) - start.charCodeAt(0) + 1));
        //             generatedPart += String.fromCharCode(randomCharCode);
        //         }
        //     } else if (type === 'n') {
        //         // 数字
        //         const startNumber = parseInt(optionArray[0] ?? 0, 10);
        //         const endNumber = parseInt(optionArray[1] ?? 9, 10);
        //         generatedPart = Math.floor(Math.random() * (endNumber - startNumber + 1)) + startNumber + '';
        //     }
        //
